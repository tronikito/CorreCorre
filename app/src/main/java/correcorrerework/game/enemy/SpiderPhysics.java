...
public class SpiderPhysics {
    ...
    public SpiderPhysics (int x, int y, int w, int h) {
        ...
        this.hitBox.left = this.r.left + sizeDot;
        this.hitBox.right = this.hitBox.left + 1;
        this.hitBox.top = this.r.top;
        this.hitBox.bottom = this.hitBox.top + sizeDot;

        pLeft = new Rect();
        pLeft.left = this.r.left;;
        pLeft.right = pLeft.left + 1;
        pLeft.top = this.r.top + sizeDot/3;
        pLeft.bottom = pLeft.top  + sizeDot/3;

        pRight = new Rect();
        pRight.left = this.r.left + 2*sizeDot;
        pRight.right = pRight.left + 1;
        pRight.top = this.r.top + sizeDot/3;
        pRight.bottom = pRight.top + sizeDot/3;

        pTop = new Rect();
        pTop.left = this.r.left + sizeDot/2;
        pTop.right = pTop.left + sizeDot;
        pTop.top = this.r.top;
        pTop.bottom = pTop.top + 1;

        pBottom = new Rect();
        pBottom.left = this.r.left + sizeDot/2;
        pBottom.right = pBottom.left + sizeDot;
        pBottom.top = this.r.top + sizeDot;
    ...
    public synchronized void checkCollisionPenguin(Rect pen) {
        if (!penguin.getImmunity()) {
            if (Rect.intersects(this.hitBox, pen)) {
                scoreboard.setLife(scoreboard.getLife() - 1);//how many DAMAGE
