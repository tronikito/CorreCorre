...
public class PlantPhysics {
    ...
    public PlantPhysics(Main m, MatrixX ma, int xPos, int yPos, int w, int h) {
        main = m;
        matrixX = ma;

        this.hitBox = new Rect();
        this.hitBox.left = yPos + matrixX.getSize()/2 + matrixX.getSize()/3;
        this.hitBox.right = this.hitBox.left + 1;
        this.hitBox.top = xPos + matrixX.getSize()/3;
        this.hitBox.bottom = this.hitBox.top + matrixX.getSize()*3;
        ...
        pLeft.top = this.r.top + this.size;
        pLeft.bottom = pLeft.top + this.size + this.size/2;

        pRight = new Rect();
        pRight.left = this.r.left + this.size + this.size/2;
        pRight.right = pRight.left + 1;
        pRight.top = this.r.top + this.size;
        pRight.bottom = pRight.top + this.size + this.size/2;

        pTop = new Rect();
        pTop.left = this.r.left + this.size/3;
        pTop.right = pTop.left + this.size;
        pTop.top = this.r.top+ this.size/3;
        pTop.bottom = pTop.top + 1;

        pBottom = new Rect();
        pBottom.left = this.r.left + this.size/3;
        pBottom.right = pBottom.left + this.size;
        pBottom.top = this.r.top + 3*this.size;
