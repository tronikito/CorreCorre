...
public class UnicornBullet extends BulletSprites implements Bullet {
    ...
    public UnicornBullet(Rect pen, boolean rLeft, boolean rRight, int orientation, int[] speed) {
    ...
    public synchronized void printBullet(Canvas c) {
        ...
        if (orientation == 1) {
            ...
            if (firstTime) {
                this.r.top = this.r.top - sizeDot;
                this.r.bottom = this.r.bottom - sizeDot;
                firstTime = false;
            }
        ...
        if (penguin) {
            if (enemyList != null) {
                for (int x = 0; x < enemyList.size(); x++) {
                    if (Rect.intersects(this.r, enemyList.get(x).getRect())) {//getHitBox()
